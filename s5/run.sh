...
# Copyright Petr Mizera Apache 2.0
...
utils/prepare_lang.sh --position-dependent-phones false --num-sil-states 3  \
      $WORK/local/dict '_SIL_' $WORK/local/lang $WORK/lang || exit 1
local/create_G.sh $WORK/lang "$LMs" $WORK/local/lm $WORK/local/dict/lexicon.txt || exit 1

#-------------------------
# The following commands are for phone decoder
local/create_phones_cs_transcription.sh "$LMs" "$TEST_SETS"
local/prep_dict.sh
utils/prepare_lang.sh --position-dependent-phones false --num-sil-states 3 \
      lang_prep/local/dict_phone "_SIL_" lang_prep/local/lang_tmp_phone_bg lang_prep/lang_phone_bg
local/vystadial_cz_format_data.sh || exit 1;
#-------------------------
  ...
  for lm in $LMs; do
    tgt_dir=${s}_`basename "$lm"`
      for f in wav.scp utt2spk spk2utt trans.txt; do
        cp $WORK/local/$s/$f $WORK/$tgt_dir
      done
      cp $WORK/$tgt_dir/trans.txt $WORK/$tgt_dir/text
...
#     echo "cp $WORK/local/$s/cmvn.scp $WORK/$tgt_dir/cmvn.scp"
  ...
  for lm in $phoneLMs; do
    tgt_dir=${s}_`basename "$lm"`
      for f in wav.scp utt2spk spk2utt trans.txt; do
        cp $WORK/local/$s/$f $WORK/$tgt_dir
      done
      cp $WORK/local/$s/text_phones $WORK/$tgt_dir/text
#     echo "cp $WORK/local/$s/feats.scp $WORK/$tgt_dir/feats.scp"
      cp $WORK/local/$s/feats.scp $WORK/$tgt_dir/feats.scp
#     echo "cp $WORK/local/$s/cmvn.scp $WORK/$tgt_dir/cmvn.scp"
      cp $WORK/local/$s/cmvn.scp $WORK/$tgt_dir/cmvn.scp
...
steps/train_mmi.sh --cmd "$train_cmd" $WORK/train $WORK/lang $EXP/tri2b_ali $EXP/tri2b_denlats $EXP/tri2b_mmi || exit 1;

echo "Train MMI on top of LDA+MLLT with boosting. train_mmi_boost is a e.g. 0.05"
steps/train_mmi.sh --cmd "$train_cmd" --boost ${train_mmi_boost} $WORK/train $WORK/lang \
   $EXP/tri2b_ali $EXP/tri2b_denlats $EXP/tri2b_mmi_b${train_mmi_boost} || exit 1;

echo "Train MPE."
steps/train_mpe.sh --cmd "$train_cmd" $WORK/train $WORK/lang $EXP/tri2b_ali $EXP/tri2b_denlats $EXP/tri2b_mpe || exit 1;

#######################################################################
#                       Building decoding graph                       #
#######################################################################

for lm in $LMs $phoneLMs; do
  lm=`basename "$lm"`
...
for s in $TEST_SETS ; do
  for lm in $LMs $phoneLMs; do
    ...
    steps/decode.sh --stage "1" --scoring-opts "--min-lmw $min_lmw --max-lmw $max_lmw" \
